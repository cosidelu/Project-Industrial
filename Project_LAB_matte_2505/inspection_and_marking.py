...
def mark_defect(controller,
    ...
    3. Calcola la posa del marker sul difetto.
    4. Calcola un punto di approccio più lontano, sulla stessa direzione.
    5. Va al punto di approccio con move_ptp.
    6. Avanza linearmente al difetto con move_line.
    7. Torna indietro linearmente al punto di approccio.
    """
    ...
    # Movimento lineare lento fino al difetto.
...
def refine_and_mark_phase(controller,
    ...
    for i, d in enumerate(unique_defects):
        ...
        try:
            # Transito sicuro con camera.
            safe_transit_via_hub(
                controller=controller,
                helmet_center=helmet_center,
                tool_pose_ee=vb.CAMERA_POSE_EE
            )
...
def main():
    ...
    1. Connette robot.
    2. Inizializza ZED.
    3. Porta robot in posizione iniziale.
    4. Esegue ispezione globale.
    5. Mostra difetti trovati.
    6. Chiede conferma operatore.
    7. Raffina e marca.
    8. Chiude tutto in sicurezza.
    """
    ...
    try:
        ...
        print("\n=== RISULTATI ISPEZIONE ===")
        print(f"Difetti trovati: {len(unique_defects)}")

        for i, d in enumerate(unique_defects):
            print(f"  Difetto {i + 1}: {np.round(d.pos3d_global, 1)} mm")

        if len(unique_defects) == 0:
            print("Nessun difetto rilevato. Fine ciclo.")
            return

        print("\nControlla i risultati prima di procedere.")
        input("Premi INVIO per iniziare raffinamento e marcatura...")

        print("\n=== FASE 2: RAFFINAMENTO + MARCATURA ===")

        refine_and_mark_phase(
            controller=controller,
            zed=zed,
            runtime=runtime,
            image_zed=image_zed,
        ...
        try:
            ...
            print(f"Errore durante il ritorno alla posizione iniziale: {e}")

        try:
            controller.disconnect()
            print("Robot disconnesso.")
        except Exception as e:
            print(f"Errore durante disconnessione robot: {e}")

        try:
            zed.close()
            print("ZED chiusa.")
        except Exception as e:
            print(f"Errore durante chiusura ZED: {e}")
