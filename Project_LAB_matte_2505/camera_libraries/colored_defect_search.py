...
CIRCULAR_MASK_RADIUS_RATIO = 0.2  # Radius as ratio of image diagonal  # ADDED
CIRCULAR_MASK_ENABLED = True  # ADDED

# Cached circular mask (will be initialized on first use)
_cached_circular_mask = None  # ADDED
_cached_mask_shape = None  # ADDED

# 3D point extraction parameters
MAX_3D_SAMPLES = 200  # Maximum number of pixels to sample for 3D points (for speed)  # ADDED
...
def get_circular_mask(image_height, image_width):  # ADDED
    """
    Get or create a circular binary mask centered on the image.
    The mask is cached to avoid recalculation every frame.
    ...
    """
    global _cached_circular_mask, _cached_mask_shape  # ADDED
    
    current_shape = (image_height, image_width)  # ADDED
    
    # Return cached mask if dimensions match
    if _cached_circular_mask is not None and _cached_mask_shape == current_shape:
        return _cached_circular_mask  # ADDED
    
    # Create new circular mask
    center_x = image_width // 2  # ADDED
    center_y = image_height // 2  # ADDED
    
    # Calculate radius based on image diagonal
    diagonal = np.sqrt(image_height**2 + image_width**2)  # ADDED
    radius = int(diagonal * CIRCULAR_MASK_RADIUS_RATIO)  # ADDED
    
    # Create an empty mask
    mask = np.zeros((image_height, image_width), dtype=np.uint8)  # ADDED
    
    # Draw a filled circle
    cv2.circle(mask, (center_x, center_y), radius, 255, -1)  # ADDED
    
    # Cache the mask
    _cached_circular_mask = mask  # ADDED
    _cached_mask_shape = current_shape  # ADDED
...
def find_green_mask_and_centroid(bgr_image):
    ...
    # Apply circular mask to limit search area
    if CIRCULAR_MASK_ENABLED:
        circular_mask = get_circular_mask(bgr_image.shape[0], bgr_image.shape[1])  # ADDED
        mask = cv2.bitwise_and(mask, circular_mask)  # ADDED

    # Optional mask cleaning with optimized morphological operations
    # Use smaller kernel (3x3) for faster processing  # MODIFIED
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))  # MODIFIED
    # Combined morphological operation for faster processing  # MODIFIED
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)  # MODIFIED
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)  # MODIFIED
...
def extract_3d_points_from_mask(mask, point_cloud):
    ...
    Goal:
    Convert green pixels into valid 3D points.

    Important note:
    Do NOT use only one pixel if you want a stable 3D estimate, but collect many 
    3D points and average them. For speed, samples pixels intelligently.

    1. Get all pixel coordinates from the binary mask
    2. Sample pixels (for speed) and read the corresponding 3D point from the ZED point cloud
    ...
    ys, xs = np.where(mask > 0)  # ADDED
    
    if len(xs) == 0:  # ADDED
        return np.empty((0, 3), dtype=np.float32)  # ADDED

    # Sample pixels if there are too many (for speed optimization)
    if len(xs) > MAX_3D_SAMPLES:  # ADDED
        # Randomly sample a subset of pixels
        indices = np.random.choice(len(xs), MAX_3D_SAMPLES, replace=False)  # ADDED
        xs = xs[indices]  # ADDED
        ys = ys[indices]  # ADDED

    points_3d = []  # ADDED

    # For each (sampled) green pixel, query the corresponding 3D point
...
def draw_debug(bgr_image, mask, centroid, contour, mean_point_3d):
    ...
    if CIRCULAR_MASK_ENABLED:
        center_x = bgr_image.shape[1] // 2  # ADDED
        center_y = bgr_image.shape[0] // 2  # ADDED
        diagonal = np.sqrt(bgr_image.shape[0]**2 + bgr_image.shape[1]**2)  # ADDED
        radius = int(diagonal * CIRCULAR_MASK_RADIUS_RATIO)  # ADDED
        cv2.circle(debug, (center_x, center_y), radius, (255, 0, 255), 2)  # ADDED
