...
def variable_helmet_radius(alpha, beta, r_apex=400.0, r_side=300.0, r_back=250.0, r_min=200.0):
    """
    ...
    MODELLO (forma quadratica liscia)
        r(a, b) = r_apex
                  ...
                  - (r_apex - r_back) * cos^2(beta)    # riduzione verso retro/fronte

    PUNTI DI ANCORAGGIO (con i default richiesti):
        - alpha=0,    beta=90  -> r_apex = 400   (apice del casco)
        - alpha=+-90, beta=90  -> r_side = 300   (lati)
        - alpha=0,    beta=0   -> r_back = 250   (retro; per simmetria anche fronte)

    NOTE
    - sin^2(alpha): 0 ad alpha=0, 1 ad alpha=+-90 -> sposta dal valore apice a quello laterale.
    - cos^2(beta):  0 a beta=90, 1 a beta=0/180   -> sposta dal valore apice a quello retro/fronte.
      Il fronte (beta verso 180) è trattato simmetricamente al retro; se serve un
      comportamento diverso davanti, sganciare i due casi.
    ...
    a = np.radians(alpha)
    b = np.radians(beta)
    r = r_apex - (r_apex - r_side) * np.sin(a) ** 2 - (r_apex - r_back) * np.cos(b) ** 2
    return max(r, r_min)
