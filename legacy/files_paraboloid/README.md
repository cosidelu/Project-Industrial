...
- `variable_helmet_radius(alpha, beta, r_apex=400, r_side=300, r_back=250, r_min=200)`: Legge del raggio di lavoro **variabile** in funzione degli angoli sferici. Restituisce un raggio (mm) più ampio all'apice e più stretto ai lati e su retro/fronte, secondo `r = r_apex - (r_apex-r_side)·sin²(alpha) - (r_apex-r_back)·cos²(beta)`. Serve a compensare l'abbassamento del centro del casco: l'apice si allontana e va tenuto a distanza maggiore per evitare collisioni. Passa per i tre ancoraggi (apice, lati, retro) = (400, 300, 250) mm di default.
- `make_radius_fn(radius)`: Normalizza il parametro `radius` in una funzione `radius(alpha, beta)`; accetta indifferentemente uno scalare (raggio costante) o una funzione già dipendente dagli angoli.
...
- `move_circle_spherical(controller, end_sph_coord, radius, tool_pose_ee, helmet_center, speed)`: Funzione master di movimento. Il parametro `radius` può essere uno **scalare** (raggio costante) o una **funzione** `radius(alpha, beta)` (es. `variable_helmet_radius`), che viene valutata punto per punto lungo la traiettoria. Prende in carico lo spostamento calcolando e validando dinamicamente la traiettoria migliore:
...
success = move_circle_spherical(
    ...
    radius=variable_helmet_radius,   # raggio variabile (alpha, beta); in alternativa uno scalare es. 400
