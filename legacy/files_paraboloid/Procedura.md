...
- **`spherical_movement.py`**: Modulo per il moto sicuro e l'elusione delle collisioni. Consente al robot di ruotare fluidamente sulla superficie del casco evitando matematicamente l'ingombro della base o del viso, dividendo in autonomia traiettorie ampie o deviandole per l'apice. Il raggio di lavoro non è costante ma definito da `variable_helmet_radius(alpha, beta)`: poiché il centro del casco (`HELMET_CENTER_GLOBAL`) è stato abbassato per raggiungere i difetti bassi, all'apice serve un raggio maggiore (per non collidere con la calotta) e ai lati/retro uno minore. Il parametro `radius` di `move_circle_spherical` accetta sia uno scalare (raggio costante) sia questa funzione.
...
for point in inspection_points:
    ...
    move_circle_spherical(
        ...
        end_sph_coord=point,
        radius=variable_helmet_radius,  # raggio variabile (alpha, beta): ampio all'apice, stretto ai lati/retro
...
move_circle_spherical(
    ...
    radius=variable_helmet_radius,  # stessa legge di raggio variabile usata in ispezione
    tool_pose_ee=vb.CAMERA_POSE_EE,
    helmet_center=vb.HELMET_CENTER_GLOBAL
)
```
